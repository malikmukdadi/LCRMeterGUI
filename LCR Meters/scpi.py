''' SCPI 'driver' for the BK Precision 889B LCR Meter
...
FREQUENCIES = frozenset(['100', '120', '1000', '10000', '100000', '200000'])
MEASURES = frozenset(['L', 'C', 'R', 'Z', 'DCR'])
SECONDARY_MEASURES = frozenset(['D', 'Q', 'TD', 'TR', 'ESR'])
...
def parse(cmd_result):
    ...
    for data in split_input:
        if data == 'N' or data.startswith('----') or data == '':
            result.append(None)
...
class ScpiConnection(object):
    ...
    def auto_fetch(self, quantity=0):
        ...
        while True:
            result = parse(self.con.readline().decode())
    ...
    def fetch(self):
        ...
        if COMMAND[2] == 'R' or COMMAND[2] == 'X':
            COMMAND[3] = COMMAND[1]

        listToStr = ''.join(map(str, COMMAND))

        self.sendcmd(listToStr)
        # print(listToStr)
        return self.sendcmd('READ?')
    ...
    def set_primary(self, param):
        ...
        if param not in MEASURES:
            raise ScpiException('''Valid primary parameters are 'L', 'C', 'R', 'Z', or 'DCR'.''')
