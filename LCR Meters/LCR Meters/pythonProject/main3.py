...
instrument = scpi.connect("COM4", post_command_delay= 500)  # Windows users can just pass in 'COMx'
...
filename = "testruns3/" + sys.argv[1] + ".xlsx"  # Enter filename here

row1 = 1 # Rows are for each rotation for measurements, in this case between C/D -> Z/THETA -> DCR
row2 = 1 # Z/THETA
row3 = 1 # DCR
...
try:
    while True:
        ...
        firstInterval = time.time() + 5*60 # First measurement set to 5 minutes
        while time.time() < firstInterval: # Loop for 5 minutes
            ...
            while temp == "":
                ...
                print("Couldn't get fetch data, trying again")
                firstInterval += 5
            ...
            worksheet.write(row1, 2, temp[1])
            worksheet.write(row1, 3, temp[2])
            worksheet.write(row1, 4, temp[0])
        ...
        #  time.sleep(1)
        instrument.set_secondary('TD') # Sets secondary to phase angle
        print("Secondary reading set to: Phase Angle")
        #  time.sleep(1)

        secondInterval = time.time() + 30 # Second measurement set for 30 seconds
        while time.time() < secondInterval: # Same deal as the first loop
            ...
            worksheet.write(row2, 7, temp[1])
            worksheet.write(row2, 8, temp[2])
            worksheet.write(row2, 9, temp[0])
            row2 += 1
        ...
        thirdInterval = time.time() + 30  # Third measurement set for 30 seconds
        while time.time() < thirdInterval:  # Same deal as the first loop
            ...
            while temp == "":
                ...
                thirdInterval += 5
            print(temp)
            worksheet.write(row3, 11, time.time() - startTime)
            worksheet.write(row3, 12, temp[1])
            worksheet.write(row3, 13, temp[0])
            row3 += 1
